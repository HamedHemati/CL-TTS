...
class Tacotron2Loss:
    ...
    def __call__(self,
        ...
        l1_loss = self.l1_criterion(postnet_outputs, mel) + \
                  self.l1_criterion(outputs, mel)
        mse_loss = self.mse_criterion(postnet_outputs, mel) + \
                   self.mse_criterion(outputs, mel)
        ...
        if self.reduction == "none":
            ...
            bce_loss = bce_loss.mul(
                logit_weights.squeeze(-1)
            ).masked_select(masks.squeeze(-1)).sum()
