...
from .attention import LSAttention as LSA
from .attention import GMMAttentionV2, ForwardAttention
...
class Decoder(nn.Module):
    def __init__(self, 
        ...
        elif self.attn_type == "GMMv2":
            self.attention_layer = GMMAttentionV2(attention_rnn_dim, 
                                                  attention_params["gmm_k"])
    ...
    def initialize_decoder_states(self, memory):
        ...
        attention_hidden = torch.zeros(B, self.attention_rnn_dim,
                                       dtype=dtype, device=device)
        attention_cell = torch.zeros(B, self.attention_rnn_dim,
                                     dtype=dtype, device=device)

        decoder_hidden = torch.zeros(B, self.decoder_rnn_dim,
                                     dtype=dtype, device=device)
        decoder_cell = torch.zeros(B, self.decoder_rnn_dim,
                                   dtype=dtype, device=device)

        attention_context = torch.zeros(B, self.encoder_embedding_dim,
                                        dtype=dtype, device=device)

        return (attention_hidden, attention_cell,
                decoder_hidden, decoder_cell, attention_context)
    ...
    def decode(self, 
        ...
        attention_hidden, attention_cell = \
            self.attention_rnn(cell_input, (attention_hidden, attention_cell))
        attention_hidden = \
            F.dropout(attention_hidden, self.p_attention_dropout, self.training)

        attention_context, attention_weights = \
            self.attention_layer(attention_hidden, encoder_outputs, mask)

        decoder_input = torch.cat((attention_hidden, attention_context), -1)

        decoder_hidden, decoder_cell = \
            self.decoder_rnn(decoder_input, (decoder_hidden, decoder_cell))
        decoder_hidden = \
            F.dropout(decoder_hidden, self.p_decoder_dropout, self.training)

        decoder_hidden_attention_context = \
            torch.cat((decoder_hidden, attention_context), dim=1)
    ...
    def forward(self, encoder_outputs, decoder_inputs, input_lengths):
        ...
        mel_outputs, gate_outputs, alignments = \
            self.parse_decoder_outputs(torch.stack(mel_outputs),
    ...
    def infer(self, 
        ...
        mel_lengths = torch.zeros([encoder_outputs.size(0)],
                                  dtype=torch.int32,
                                  device=encoder_outputs.device)
        not_finished = torch.ones([encoder_outputs.size(0)],
                                  dtype=torch.int32,
                                  device=encoder_outputs.device)
